...
(defmethod name-matches :around ((query t) (name t) &key start1 end1 start2 end2)
  (flet ((check-bounding-indices (sequence start end)
           (let* ((length (length (#+sbcl progn #-sbcl name-components sequence)))
                  (start  (or start 0))
                  (end    (or end length)))
      ...
      (map-matching-options
       (lambda (option &rest args)
         ...
         (let* ((name/old (option-name option))
                (name/new #+sbcl (subseq name/old (1- length))
                          #-sbcl (make-name (subseq (name-components name/old) (1- length)))))
